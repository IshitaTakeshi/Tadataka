...
fn project_vecs(py: Python<'_>, xs: &PyArray2<f64>) -> Py<PyArray2<f64>> {
    projection::project_vecs(xs.as_array()).into_pyarray(py).to_owned()
}

#[pyfunction]
fn inv_project_vec(py: Python<'_>, x: &PyArray1<f64>, depth: f64)
    -> Py<PyArray1<f64>> {
...
#[pyfunction]
fn inv_project_vecs(py: Python<'_>, xs: &PyArray2<f64>, depths: &PyArray1<f64>)
    -> Py<PyArray2<f64>> {
